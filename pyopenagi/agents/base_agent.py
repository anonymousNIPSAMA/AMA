...
class BaseAgent:
    ...
    def check_workflow(self, message):
        try:
            ...
            workflow = eval_from_json(message)
            ...
            print(f"Error in check_workflow: {e}")
            return None
    ...
    def automatic_workflow(self):
        for i in range(self.plan_max_fail_times):
            ...
            if workflow:
                ...
                time.sleep(2)
                self.logger.log(
                    f"Failed to get a valid workflow. Retrying... ({i+1}/{self.plan_max_fail_times})\n",level='info')
        
        self.logger.log(
            f"Failed to get a valid workflow after {self.plan_max_fail_times} attempts. Exiting...\n",
            level='info'
        )
        return None
